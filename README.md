...
- Brief exaplanation about the codes
  ...
  - Source: In the ```.cpp``` file, first it needs to setup and then goes into a ```while``` loop implementing other three basic functions in it. For a comfortable display, it includes a ```Linux```'s sleep() following the ```Draw()```.
 - Makefile
  - Just simply apply ```$ make``` in the command line then it will generate ```.o``` files and the executable program.
